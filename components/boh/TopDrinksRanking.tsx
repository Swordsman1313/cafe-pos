...
export default function TopDrinksRanking({
  ...
  return (
    ...
    >
      {/* ── Fixed Header (Clean, no subtitle) ── */}
      <div className="shrink-0 mb-2.5">
        ...
        {categories.length > 2 && (
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar scrollbar-none flex-nowrap py-1 mb-0.5">
            {categories.map((cat) => (
              <button
                key={cat}
                type="button"
      ...
      <div className="relative flex-1 min-h-0">
        ...
        <div
          ref={listRef}
          className="h-full overflow-y-auto pr-1.5 py-1 space-y-2 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700 hover:scrollbar-thumb-slate-300"
        >
          {filteredProducts.map((item, idx) => {
            ...
            return (
              <div
                key={item.name}
                className={`p-2.5 rounded-2xl border transition-all hover:scale-[1.01] max-w-full overflow-hidden space-y-1.5 ${
                  isLight
                    ? isTop3
              ...
              >
                {/* ── Line 1: [Rank Badge] [Drink Name] [Category Pill] (Left) | [USD Price] [KHR Sub-price] (Right) ── */}
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0 pr-1">
                    <span className="text-sm shrink-0 select-none">
                      {medals[idx] ?? (
                        <span className="text-[11px] font-black text-slate-400 w-4 text-center inline-block">
                          #{idx + 1}
                        </span>
                      )}
                    </span>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className={`font-medium text-[10px] block ${isLight ? "text-slate-600" : "text-slate-400"}`}>
                      {item.revenueKHR.toLocaleString()} ៛
                    </span>
                  </div>
                </div>

                {/* ── Line 2: [Units Sold] • [% Mix] (Left) | [Slim Progress Bar] (Full Width) ── */}
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-[10.5px]">
                    <span className="font-semibold text-slate-500 dark:text-slate-400">
                      {item.quantity} sold <span className="text-slate-400 dark:text-slate-500">·</span> {item.shareOfTotalPct}% mix
                    </span>
                    <span className="font-bold text-amber-700 dark:text-amber-400 text-[10px]">
                      {item.relativeVolumePct}% vol
                    </span>
                  </div>
      ...
      {/* ── Fixed Footer ── */}
      <div
        className={`shrink-0 pt-2.5 border-t mt-2 text-[10.5px] flex items-center justify-between ${
