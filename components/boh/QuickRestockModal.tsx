...
interface QuickRestockModalProps {
  ...
  onClose: () => void;
  onConfirmRestock: (ingredientId: string, addedAmount: number, isDirectReplenish: boolean) => void;
  isLight?: boolean;
}
...
export default function QuickRestockModal({
  ...
  const handleCreatePO = () => {
    setIsSuccess(true);
    setSuccessMessage(`Purchase Order #PO-${Date.now().toString().slice(-5)} created for ${ingredient.supplierName}!`);
    setTimeout(() => {
      onConfirmRestock(ingredient.id, 0, false);
      onClose();
    }, 1500);
  };
  ...
  const handleQuickReplenish = () => {
    setIsSuccess(true);
    setSuccessMessage(`Logged immediate delivery! ${ingredient.name} replenished to full capacity.`);
    setTimeout(() => {
      onConfirmRestock(ingredient.id, ingredient.capacity, true);
      onClose();
    }, 1500);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div
        className={`w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-200 ${
      ...
      >
        ...
        <div className="flex items-center justify-between border-b pb-3 border-slate-200/60 dark:border-slate-800">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-7 w-7 rounded-full flex items-center justify-center text-slate-400 hover:text-slate-700 dark:hover:text-white bg-slate-100 dark:bg-slate-800"
          >
            <X size={14} />
          </button>
        </div>
        ...
        {isSuccess ? (
          <div className="py-8 text-center space-y-2 animate-in zoom-in-95">
            <div className="h-12 w-12 rounded-full bg-emerald-500/20 text-emerald-400 mx-auto flex items-center justify-center font-bold">
              <CheckCircle2 size={28} />
            </div>
            <h3 className="font-extrabold text-sm text-emerald-400">Action Confirmed</h3>
            <p className={`text-xs ${isLight ? "text-slate-600" : "text-slate-300"}`}>{successMessage}</p>
          </div>
        ) : (
          <>
            ...
            >
              <div className="flex items-center gap-2">
                <AlertTriangle size={16} className={ingredient.status === "critical" ? "text-rose-400" : "text-amber-400"} />
                <div>
            ...
            <div className={`p-3.5 rounded-2xl border space-y-2 text-xs ${isLight ? "bg-slate-50 border-slate-200" : "bg-slate-800/60 border-slate-700/60"}`}>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold">${(ingredient.reorderEstimatedCostUSD / (ingredient.suggestedReorderQty || 1)).toFixed(2)} / pack</span>
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-right pl-2">
                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Est. PO Total</span>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-200/60 dark:border-slate-800">
              ...
              <button
                ...
                className="w-full py-2.5 rounded-2xl bg-emerald-600/90 hover:bg-emerald-500 text-white font-black text-xs flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer"
              >
                <Truck size={15} />
