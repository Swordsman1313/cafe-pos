...
export default function HourlyBreakdownDrawer({
  ...
  const currentData = activeBucket;
  const totalRev = currentData.revenueUSD;
  const cashPct = totalRev > 0 ? Math.round((currentData.paymentSplit.cashUSD / totalRev) * 100) : 35;
  const khqrPct = totalRev > 0 ? Math.round((currentData.paymentSplit.khqrUSD / totalRev) * 100) : 55;
  ...
  return (
    <>
      ...
      >
        {/* ── Fixed Top Header (Pinned at top) ── */}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {currentData.isPeak && (
            <div className="p-3 rounded-2xl bg-gradient-to-r from-amber-500/20 via-orange-500/20 to-amber-500/10 border border-amber-500/30 flex items-center justify-between">
              ...
              <span className="text-[10.5px] font-bold text-amber-700 dark:text-amber-300 bg-amber-500/20 px-2 py-0.5 rounded-md">
                {currentData.pctOfDaily}% of Total Sales
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <div className={`p-3 rounded-2xl border ${isLight ? "bg-slate-50 border-slate-200/80" : "bg-slate-800/60 border-slate-700/60"}`}>
              ...
              <span className={`text-xl font-black block leading-tight ${isLight ? "text-slate-900" : "text-white"}`}>
                {currentData.orders} <span className="text-xs font-bold text-slate-400">tickets</span>
              </span>
              <span className={`text-[10px] font-medium ${isLight ? "text-slate-600" : "text-slate-400"}`}>
                Rate: {currentData.orderRatePerHour} ord/hr
              </span>
            </div>
            ...
            <div className={`p-3 rounded-2xl border ${isLight ? "bg-slate-50 border-slate-200/80" : "bg-slate-800/60 border-slate-700/60"}`}>
              ...
              <span className={`text-xl font-black block leading-tight ${isLight ? "text-slate-900" : "text-white"}`}>
                ${currentData.avgTicketUSD.toFixed(2)}
              </span>
              <span className={`text-[10px] font-medium ${isLight ? "text-slate-600" : "text-slate-400"}`}>
                ≈ {(currentData.avgTicketUSD * 4100).toLocaleString()} ៛
              </span>
            </div>
            ...
            <div className={`p-3 rounded-2xl border ${isLight ? "bg-slate-50 border-slate-200/80" : "bg-slate-800/60 border-slate-700/60"}`}>
              ...
              <span className="text-xl font-black text-emerald-600 dark:text-emerald-400 block leading-tight">
                {currentData.avgTransactionSpeedSec}s <span className="text-xs font-bold text-slate-400">/ ticket</span>
              </span>
              <span className={`text-[10px] font-medium ${isLight ? "text-slate-600" : "text-slate-400"}`}>
                {currentData.avgTransactionSpeedSec <= 40 ? "⚡ Fast Flow" : "Normal Flow"}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2 pt-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-500">
                Top {Math.min(currentData.topSellingItems.length, 3)}
              </span>
            </div>

            <div className="space-y-1.5">
              {currentData.topSellingItems.slice(0, 3).map((item, idx) => (
                <div
                  key={item.name}
          ...
          <div className="space-y-2 border-t pt-3 border-slate-200/60 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-500">Total: ${currentData.revenueUSD.toFixed(2)}</span>
            </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1 text-[10.5px]">
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <span className="text-[9.5px] text-slate-500 dark:text-slate-400">${currentData.paymentSplit.khqrUSD.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <span className="text-[9.5px] text-slate-500 dark:text-slate-400">${currentData.paymentSplit.cashUSD.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <span className="text-[9.5px] text-slate-500 dark:text-slate-400">${currentData.paymentSplit.cardUSD.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-2 border-t pt-3 border-slate-200/60 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold text-slate-500">
                {currentData.ticketsList?.length || 0} Tickets Logged
              </span>
            </div>

            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {(currentData.ticketsList || []).map((tix) => (
                <div
        ...
        {/* ── Fixed Bottom Action (Pinned at bottom) ── */}
